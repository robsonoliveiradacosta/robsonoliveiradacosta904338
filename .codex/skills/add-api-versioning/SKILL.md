...
| Strategy | Pros | Cons |
|---|---|---|
| **URL path** (`/api/v1/`, `/api/v2/`) ← this skill | Visible, explicit, easy to route at gateway, cacheable, browser-friendly | Two paths in OpenAPI; URL pollution |
...
| Input | Default |
|---|---|
| Current version (already in URL) | `v1` (detected from existing `@Path("/api/v1/...")`) |
...
└── resource/
    ...
    │   └── AlbumResource.java           # @Path("/api/v1/albums")
    └── v2/                              # new version lives here
        └── AlbumResource.java           # @Path("/api/v2/albums")
```
...
- **Multiple minor versions in the URL** (`/api/v1.2/`). The URL version is binary: either it's compatible or it's not. Compatibility lives in OpenAPI MINOR/PATCH.
