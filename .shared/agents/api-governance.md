...
- New endpoint: `POST /v1/albums/{id}/restore`
- New optional query param `includeDeleted` on `GET /v1/albums`
...
2. Preserve `/v1` compatibility unless a breaking change is explicitly approved.
...
Use this agent to review whether adding `releaseDate` to album responses is compatible with existing `/v1/albums` clients.
