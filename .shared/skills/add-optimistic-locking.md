...
Add JPA optimistic locking so two concurrent updates can't silently overwrite each other. After this skill, the second `PUT /api/v1/<resource>/{id}` to a stale version returns **409 Conflict** with a Problem Details body, and the caller knows to refetch + retry.
