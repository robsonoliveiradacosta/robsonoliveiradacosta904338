...
export default function (data) {
  ...
  if (Math.random() < 0.7) {
    const res = http.get(`${BASE}/api/v1/albums?page=0&size=20`, headers);
    ...
    const res = http.get(`${BASE}/api/v1/albums/${id}`, headers);
...
export default function (data) {
  http.get(`${BASE}/api/v1/albums?page=0&size=20`, authHeaders(data.token));
}
```
...
export default function (data) {
  ...
  check(http.get(`${BASE}/api/v1/albums`, headers),     { 'albums 200': r => r.status === 200 });
  check(http.get(`${BASE}/api/v1/artists`, headers),    { 'artists 200': r => r.status === 200 });
}
```
