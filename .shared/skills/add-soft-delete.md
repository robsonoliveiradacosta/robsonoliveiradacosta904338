...
- `DELETE /api/v1/albums/{id}` marks `deleted_at = NOW()` instead of removing the row.
...
void deleteIsSoft() {
    ...
    given().auth().oauth2(TestTokenHelper.adminToken())
    .when().get("/api/v1/albums/admin/all")
...
void restoreBringsItBack() {
    Long id = createAlbum();
    given().auth().oauth2(TestTokenHelper.adminToken()).delete("/api/v1/albums/" + id);
    given().auth().oauth2(TestTokenHelper.adminToken()).post("/api/v1/albums/" + id + "/restore")
        .then().statusCode(204);
